...
-- Every statement is `IF NOT EXISTS` so the schema can be re-applied to an
-- existing database: the Rust bootstrap runs this batch on startup to complete
-- a database written by an older build instead of refusing to open it.
...
CREATE UNIQUE INDEX IF NOT EXISTS product_groups_active_name_unique ON product_groups(name) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS main_categories_active_group_name_unique ON main_categories(product_group_id, name) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS workshops_active_name_unique ON workshops(name) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS users_active_username_unique ON users(username) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS products_active_code_unique ON products(product_code) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS label_templates_default_kind_unique ON label_templates(template_kind) WHERE is_default = 1 AND is_active = 1;
CREATE UNIQUE INDEX IF NOT EXISTS return_scans_accepted_product_session_unique ON return_scans(return_session_id, product_id)
  WHERE scan_status = 'accepted' AND product_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS main_categories_group_index ON main_categories(product_group_id);
CREATE INDEX IF NOT EXISTS products_group_index ON products(product_group_id);
CREATE INDEX IF NOT EXISTS products_category_index ON products(main_category_id);
CREATE INDEX IF NOT EXISTS products_workshop_index ON products(workshop_id);
CREATE INDEX IF NOT EXISTS products_template_index ON products(label_template_id);
CREATE INDEX IF NOT EXISTS package_items_package_index ON package_items(package_id);
CREATE INDEX IF NOT EXISTS package_items_product_index ON package_items(product_id);
CREATE INDEX IF NOT EXISTS return_scans_session_index ON return_scans(return_session_id);
CREATE INDEX IF NOT EXISTS return_scans_product_index ON return_scans(product_id);
